...
describe('SignatureService', () => {
  ...
  const sigB64 = cryptoSign('sha256', payload, {
    key: privateKey,
    dsaEncoding: 'der',
  }).toString('base64');
  ...
  it('rechaza payload manipulado', () => {
    expect(service.verify(spkiB64, Buffer.from('{"seq":2}'), sigB64)).toBe(
      false,
    );
  });
  ...
  it('fingerprint lanza con SPKI inválido', () => {
    expect(() => service.fingerprint('no-es-un-spki-valido')).toThrow();
  });
});
